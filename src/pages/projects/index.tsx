...
import { useRoqClient, useProjectFindManyWithCount } from 'lib/roq';
...
export function ProjectListPage(props: ProjectListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['name.contains', 'description.contains', 'status.contains'],
          ...params.filters,
        },
        'project',
      ),
  ...
  const { data, error, isLoading, mutate } = useProjectFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.project.delete({ where: { id } });
      await mutate();
    } catch (error) {
