...
import { useRoqClient, useBusinessFindFirst } from 'lib/roq';
...
function BusinessEditPage() {
  ...
  const { data, error, isLoading, mutate } = useBusinessFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: BusinessInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.business.update({
        data: values as RoqTypes.business,
        where: {
          id,
        },
      });
