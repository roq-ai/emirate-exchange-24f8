...
import { useRoqClient, useBusinessFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<BusinessInterface, unknown>;
...
export function BusinessListPage(props: BusinessListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['industry.contains', 'location.contains'],
          ...params.filters,
        },
        'business',
      ),
  ...
  const { data, error, isLoading, mutate } = useBusinessFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.business.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: BusinessInterface) => {
    if (hasAccess('business', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/businesses/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/businesses/view/${record.id}`} passHref legacyBehavior>
                <Button
