...
import { useTeamMemberFindFirst, useRoqClient } from 'lib/roq';
...
function TeamMemberViewPage() {
  ...
  const queryParams = useMemo<{ include: { user: boolean; team: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useTeamMemberFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('team_member', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/team-members/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Role" text={data?.role} />
