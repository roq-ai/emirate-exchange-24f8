...
import { useRoqPlatformUserProfiles, useProjectCount, useItAdministratorCount, useBusinessCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const projectStat = useProjectCount();

  const itAdministratorStat = useItAdministratorCount();

  const businessStat = useBusinessCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Projects',
      result: projectStat?.data,
      isLoading: projectStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'It Administrators',
      result: itAdministratorStat?.data,
      isLoading: itAdministratorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Businesses',
      result: businessStat?.data,
